...
def run():
    ...
    if bulldog_algo == DDPG:
        np.save('results1/DDPG/'+model+'/bulldogs.npy', np.array(bulldog_score_history))
    if runner_algo == DDPG:
        np.save('results1/DDPG/'+model+'/runners.npy', np.array(runner_score_history))
    np.save('results1/DDPG/'+model+'/eps.npy', np.array(episodes))
        ...
        for ALPHA in [1e-4]:
            for BETA in [1e-3, 1e-4]:
                for GAMMA in [0.95, 0.99]:
                    ...
                    if ALPHA == 1e-4 and BETA == 1e-3 and GAMMA == 0.95:
                        continue
